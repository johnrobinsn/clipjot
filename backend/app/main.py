...
@rt("/docs")
def docs_redirect():
    """Redirect to current API documentation."""
    return RedirectResponse("/docs/api/v1", status_code=303)
...
def llms_txt():
    ...
    content = api_docs.generate_llms_txt(full=False)
...
def llms_full_txt():
    ...
    from . import api_docs
    content = api_docs.generate_llms_txt(full=True)
    return Response(content, media_type="text/plain; charset=utf-8")
