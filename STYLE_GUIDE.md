...
### Status Messages in Panels/Sheets
When a panel or bottom sheet shows status messages (success, error, progress) after an action like Save, always **pre-reserve the space** for the status area to prevent the panel from resizing on state change.

- Use `android:visibility="invisible"` (not `gone`) so the view occupies layout space while hidden
- Wrap status views in a `FrameLayout` so they overlap (only one is shown at a time)
- Use a single `statusMessage` `TextView` with dynamic text color rather than separate error/success views

This avoids jarring layout shifts when the user taps Save.
